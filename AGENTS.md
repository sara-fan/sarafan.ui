...
- Treat `GET /api/v1/orders` as an authenticated, current-customer-only, newest-first list. Validate the complete response inside the authorized-request failure policy: positive unique IDs, unique order numbers, Core-owned status/currency values, HTTP(S) URLs, positive quantity and seller amount, nullable product/store/image/price fields, and calendar-valid RFC 3339 creation times. Resolve names and upper-status grouping exclusively from `GET /api/v1/orders/ops`; never compile aliases or fallback mappings. Discard completions from an older customer identity, but propagate the current request failure that invalidates its session.
...
- Validate complete legal-document DTOs on both current-kind and UUID reads, including Ops kinds/categories and the requested identity. Only explicit `document: null` means no current document. Map nested `personalDataConsent.*` field errors to the consent control through the shared field-error helper.
