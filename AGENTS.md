...
- Preserve single-flight token refresh per identity generation and retry an authorized request at most once. Discard late refresh results and failures after the identity changes. Treat only the canonical invalid-refresh-token problem type as expected session expiry; distinguish network restore failures and offer an explicit retry state.
