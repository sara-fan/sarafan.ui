...
- Preserve single-flight token refresh per identity generation and retry an authorized request at most once. Abort in-flight refreshes when identity changes, reject stale completions before the original authorized transport can retry, and discard late refresh results and failures. Treat only the canonical invalid-refresh-token problem type as expected session expiry; distinguish network restore failures and offer an explicit retry state.
- Treat network failures, HTTP 5xx responses and invalid HTTP response formats as service unavailability. Clear the authenticated session and present the single user-facing message `Сервис временно недоступен. Пожалуйста, повторите позже`. Do not expose session restoration terminology or repeat a service-unavailability title and detail.
...
- Product entry accepts absolute HTTP(S), scheme-relative, and protocol-free web addresses; protocol-free input is normalized to HTTPS and Core's anonymous no-store order preview is authoritative. Require a syntactically valid DNS hostname with an IANA-listed top-level domain, reject IP literals and URL userinfo credentials, load the versioned suffix catalogue exclusively from anonymous order Ops, and treat its maximum length as authoritative; ops-less draft checks use only the validated protocol ceiling. Never embed a second catalogue or use DNS/runtime network checks in the UI. Persist only a versioned same-tab draft (canonical URL, quantity, comment, idempotency key, resume intent and consent-bound customer) in `sessionStorage`, never in route state/query or logs. Reuse one creation key through ambiguous failures, authentication and consent renewal; regenerate it only when the effective canonical URL, quantity or trimmed comment changes. The v0.1.0 My Orders wiring is a first increment: final customer cost, quote expiry, paid amount, real ETA and complete order details remain later lifecycle work.
...
- Treat `/consents`, `/consents/personal-data`, and `/legal/:documentRef` as stable limited-access routes. Keep consent and legal failures recoverable in place, and scope asynchronous results to the current route and customer identity.
- Every user-relevant failure has one presentation owner. When authentication, session restoration and background consent loading fail from the same outage, show only the foreground owner: authentication dialog first, otherwise the shell restoration notice, otherwise consent recovery. Stores reject transport/server failures; no empty catches or unobserved promise rejections. Expected suppression uses the named shared policy.
- A foreground flow that presents consent-loading failures must acquire shared consent-notice suppression before starting the request and retain ownership until its error is cleared or the flow unmounts. Suppression is token-based so overlapping owners cannot prematurely restore the shell notice.
...
- Bind phone resolution, code requests and verification to the dialog operation's abort signal; invalidate and abort them on close, replacement or unmount. Recheck the operation after shared Ops loading and before issuing requests or accepting results. Display legal-kind names from Legal Ops. A reopened authentication dialog clears the prior session notice through a session action; the first opening (including an initially hidden dialog) must still show the failure that required authentication.
...
- Validate the full customer DTO (positive Int32 ID, canonical phone, Ops state, timestamps, photo flag and complete nullable profile) before accepting authentication or profile responses. Profile updates must retain the requested ID/phone and validate inside the request failure policy; malformed current responses clear identity and present service unavailability. Discard profile completions from an earlier identity generation, including errors, and do not present them as saved. Propagate the current request's refresh failure even when it invalidates the session. Use complete customer fixtures in transport tests and mutate them explicitly for rejection cases.
...
- Validate complete legal-document DTOs on both current-kind and UUID reads, including Ops kinds and the requested identity. Only explicit `document: null` means no current document. Map nested `personalDataConsent.*` field errors to the consent control through the shared field-error helper.
...
- Scope personal history and the nullable latest withdrawal request to the current identity, discard late results after logout, and keep legal/request access available during renewal. Present one action, `Прекратить использовать систему и отозвать согласие на хранение и обработку персональных данных`; disable it while the latest request is pending and enable it after processing. Let the current document heading use the full card width and render consent history newest-first as an unnumbered list. Do not add explanatory copy about request side effects. Future quote/contact persistence must use the same server consent gate.
...
- The public notice controller loads only legal Ops for footer document links, including anonymous visits, and retries catalogue failures without requiring a customer. Keep those legal-document links public, but show the footer `Согласия` route link only to an authenticated customer; this visibility rule does not change anonymous direct access to the limited consent routes. Present catalogue failures inline without hiding route content; load personal consent history only on consent routes. Keep this loading independent of authentication restoration and discard stale work after navigation or identity changes.
