...
- Treat `GET /api/v1/orders` as an authenticated, current-customer-only, newest-first list. Validate both its complete response and `GET /api/v1/orders/ops` inside the identity-scoped service-unavailability policy: positive unique IDs, unique order numbers, Core-owned status/currency values, HTTP(S) URLs, positive quantity and seller amount, nullable product/store/image/price fields, and calendar-valid RFC 3339 creation times. Resolve names, terminal grouping, and progress exclusively from validated Ops metadata; never compile aliases or fallback mappings. Clear and reload cards synchronously when the customer ID changes. Discard completions from an older customer identity, but propagate the current request failure that invalidates its session.
